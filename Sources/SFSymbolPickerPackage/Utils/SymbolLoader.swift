...
class SymbolLoader {
    ...
    func decodePlist<T:Decodable>(_ type: T.Type, from fileName: String) -> T {
        ...
        guard let data = try? Data(contentsOf: resourceURL) else {
            fatalError("Couldnt read \(fileName) from bundle")
        }
    ...
    func buildSymbolsArray() {
        ...
        for (name, releaseYear) in symbolAvailability.symbols {
            ...
            let osAvailableDict = symbolAvailability.year_to_release[releaseYear] ?? [:]
            let osVersions = osAvailableDict.map { os, version in
